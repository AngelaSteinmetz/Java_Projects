...
public class Actor implements Comparable<Actor>
{
    ...
    public void readMail()
    {
        ...
        {
            Message recv = mailbox.remove();
            if (recv.getText().startsWith( "need" ))
            {
                String possession = recv.getText().substring( 5 );
                if (countPossessions(possession) > 1)
                {
                    reply(recv.getSender(), "have " + possession);
                }
            }
            else if (recv.getText().startsWith( "have" ))
            {
                String possession = recv.getText().substring( 5 );
                if (countPossessions(possession) < 1)
                {
                    reply(recv.getSender(), "ship " + possession);
                }
            }
            else if (recv.getText().startsWith( "ship" ))
            {
                String possession = recv.getText().substring( 5 );
                if (countPossessions(possession) > 1)
                {
                    if (recv.getSender().receiveItem( this, possession ))
                    {
